...
4. **Log food** records a product or recipe portion. New ordinary entries automatically recalculate when their saved product, recipe, or recipe ingredients change. One-off ingredient overrides stay fixed. Existing entries created before source tracking are not backfilled. Save a different recipe as a new dish instead of changing the template followed by existing linked entries.
5. **Journal** shows a selected day, full calendar week (Monday start), calendar month, or inclusive custom range. Use the arrows/date picker to navigate and Today to return. Click an entry to open its closable detail modal. **Edit entry** can change its name, date, meal, notes, amount/unit, and component snapshots (names, grams, calories and all nutrients), including adding/removing components. Nutrition is entered as totals for each component, not per 100g; entry totals are computed from the components. Amount/unit labels do not silently change nutrition: use **Scale components to this amount** for proportional resizing, or override components directly. Changes affect this entry only, preserve its ID and retry key, and reject stale snapshot edits using `expectedRevision` (legacy entries start at 0). Actual name/quantity/component changes stop automatic source updates for this entry. Date/meal/notes-only corrections keep automatic updates. Missing-label warnings distinguish incomplete totals from complete intake.
...
Install the published skill as `calorie-ledger/SKILL.md` in the calling agent's supported skill directory. The canonical repository copy is under `.agents/skills/`; the Markdown download is generated during builds. For Claude.ai, download `/calorie-ledger-skill.zip`, enable code execution in **Settings → Capabilities**, then **Customize → Skills → + → Create skill → Upload a skill**, upload the ZIP and enable it. Keep the MCP connector enabled separately: the skill supplies instructions, the connector supplies authenticated tools. Re-upload the ZIP after skill updates. See [Claude's current installation guide](https://support.claude.com/en/articles/12512180-use-skills-in-claude). Regenerate the checked-in ZIP after editing the skill: from `.agents/skills`, run `zip -X ../../public/calorie-ledger-skill.zip calorie-ledger/SKILL.md`. Use OAuth for Claude's connector or explicit bearer headers for clients that support them.

Source tracking uses nullable `entries.source_input` and `entry_dependencies(user_id, kind, source_id, entry_id)`, whose primary key supports source-to-entry lookups. Dish entries link to both their recipe and its products; recipe edits replace those dependencies. Recalculation shares the logging calculator and runs inside the existing per-user transaction; no journal-wide scan or background sync is needed. Revisions advance so stale journal edits fail safely, and retry keys remain unchanged. Removing required calories/portions/yield rejects the source edit atomically. Deleting a source freezes its linked entries. Actual journal name/quantity/component overrides detach the entry; date/meal/notes edits keep it linked. Work scales with affected entries, so exceptionally large histories may eventually need an asynchronous revisioned update design.
